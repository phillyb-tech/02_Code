...
if __name__ == "__main__":
    ...
    financial_table_data.append({
        ...
        "Scans per Hour (per scanner)": f"{baseline_scans/6/24:.1f}",
        "Scans per Day (per scanner)": f"{baseline_scans/6:.1f}",
        "Add'l Scans/Hour (per scanner)": "0.0",
        "Add'l Scans/Day (per scanner)": "0.0",
        "Add'l Scans/Day (Total)": "0.0",
        "Monthly Add'l CM": "$0",
        "Annual Add'l CM": "$0",
        "Avg Robot Wait (min)": "0.0",
        "Robot Util %": "0.0%",
        "Robot Idle (hrs)": "0.0",
    })
    ...
    for scenario_key, scenario_label in scenarios_to_test:
        ...
        # Get theoretical savings for this scenario
        theoretical_time_saved_per_exam = theoretical_savings[scenario_key]
        # Calculate theoretical daily savings based on actual completed scans
        theoretical_daily_savings = theoretical_time_saved_per_exam * completed_scans
        ...
        # Calculate idle time reduction (for reporting only - doesn't create revenue)
        idle_time_reduction_per_scanner = baseline_idle - idle_time
        
        # Calculate financial impact from ACTUAL additional scans completed
        # Only count scans that actually happened in the simulation
        daily_additional_cm = actual_additional_scans * CT_CM_PER_SCAN
        monthly_additional_cm = daily_additional_cm * 30  # Monthly estimate
        
        # Note: Idle time reduction shows potential, but revenue only comes from actual scans
        total_additional_scans = actual_additional_scans  # Use real results, not theoretical

        # Calculate actual hourly improvement based on real simulation results
        # Distribute actual additional scans across time periods
        if actual_additional_scans > 0:
            daytime_hours = 12  # 7am-7pm: high demand period
            daytime_additional_scans = actual_additional_scans * 0.70
            peak_additional_scans_per_hour_all_scanners = daytime_additional_scans / daytime_hours
            peak_additional_scans_per_hour_per_scanner = peak_additional_scans_per_hour_all_scanners / 6
        else:
            peak_additional_scans_per_hour_per_scanner = 0.0
        ...
        financial_table_data.append({
            ...
            "Scans per Hour (per scanner)": f"{completed_scans/6/24:.1f}",
            "Scans per Day (per scanner)": f"{completed_scans/6:.1f}",
            "Add'l Scans/Hour (per scanner)": f"{peak_additional_scans_per_hour_per_scanner:.1f}",
            ...
            "Avg Robot Wait (min)": f"{robot_wait:.1f}",
            "Robot Util %": f"{robot_utilization*100:.1f}%",
            "Robot Idle (hrs)": f"{idle_time_between_transports/60:.1f}",
        })

        # Show actual simulation results vs theoretical potential
        theoretical_capacity_freed = idle_time_reduction_per_scanner / 12.11 * 6  # Potential scans from idle reduction
        ...
        efficiency_table_data.append({
            ...
            "Add'l Daily Scans": f"{actual_additional_scans:.1f} actual (vs {theoretical_capacity_freed:.1f} theoretical capacity)",
    ...
    # Custom header formatting for Table 1 with proper column widths
    print(f"{'Scenario':<42} {'Scans per':<12} {'Scans per':<12} {'Add\'l Scans/':<12} {'Add\'l Scans/':<12} {'Add\'l Scans/':<12} {'Monthly':<12} {'Annual':<13} {'Avg Robot':<10} {'Robot':<8} {'Robot':<8}")
    print(f"{'':42} {'Hour (per':<12} {'Day (per':<12} {'Hour (per':<12} {'Day (per':<12} {'Day (Total)':<12} {'Add\'l':<12} {'Add\'l':<13} {'Wait':<10} {'Util':<8} {'Idle':<8}")
    print(f"{'':42} {'scanner)':<12} {'scanner)':<12} {'scanner)':<12} {'scanner)':<12} {'':12} {'CM':<12} {'CM':<13} {'(min)':<10} {'%':<8} {'(hrs)':<8}")
    print("-"*171)
    
    for row in financial_table_data:
        print(f"{row['Scenario']:<42} {row['Scans per Hour (per scanner)']:<12} {row['Scans per Day (per scanner)']:<12} {row['Add\'l Scans/Hour (per scanner)']:<12} {row['Add\'l Scans/Day (per scanner)']:<12} {row['Add\'l Scans/Day (Total)']:<12} {row['Monthly Add\'l CM']:<12} {row['Annual Add\'l CM']:<13} {row['Avg Robot Wait (min)']:<10} {row['Robot Util %']:<8} {row['Robot Idle (hrs)']:<8}")
